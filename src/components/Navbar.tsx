...
import { Shield, ShieldAlert, FlaskConical, BookOpen, Home, Swords, ListOrdered, Terminal } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-50 px-4 pt-4 md:px-6">
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 rounded-lg border border-[color:var(--bg-border)] bg-[color:var(--bg-card)]/90 px-4 py-2.5 shadow-[0_8px_32px_-12px_rgb(0_0_0/0.5)] backdrop-blur-md">
        ...
        <Link
          to="/lab"
          className={`${labCtaBase} hidden bg-[color:var(--cyan-neon)] px-3.5 py-2 text-xs text-[color:var(--bg-base)] shadow-[0_0_24px_-6px_var(--cyan-neon)] hover:bg-[#7dd3fc] md:inline-flex`}
          activeProps={{
            className: `${labCtaBase} hidden bg-[color:var(--cyan-neon)] px-3.5 py-2 text-xs text-[color:var(--bg-base)] shadow-[0_0_24px_-6px_var(--cyan-neon)] ring-2 ring-[color:var(--cyan-neon)]/40 ring-offset-2 ring-offset-[color:var(--bg-card)] md:inline-flex`,
          }}
        >
